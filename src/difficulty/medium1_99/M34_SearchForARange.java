 ...
 * @author William
 *
 *         从题目中获取到关键字：sorted array ，find...position, a given target value,
 *         O(logn).
 * 
 *         这些关键字都在提醒我们这道题的思路应该是二分查找法。
 * 
 *         二分查找方法, 二分查找经常用来在有序的数列查找某个特定的位置。
 * 
 *         因此，应用二分查找法，这个数列必须包含以下特征： 1存储在数组中 2有序排列
 * 
 *         同时这道题让我们确定 starting position和ending position，这就让我们联想到之前做过的Search
 *         Insert Position一题，当无法查找到given target时，
 *         利用非递归二分查找法所得的最终low和high指针，将会指向该无法查找到的元素的左右两个元素。说不清楚看例子，
 * 
 *         例如，给定arraylist [1,2,4,5]
 *         target为3，那么通过传统非递归二分查找法，low指针将会指向4（位置为2），high指针指向2（位置为1）。
 * 
...
public class M34_SearchForARange {
